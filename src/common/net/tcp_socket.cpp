...
int recv_timeout(int socket, void *buf, int len, unsigned int timeout) {
	...
	while (1) {
		...
		if (recv_result < 0) {
			logmsg(lt_error,"recv(%i,((char*)%x)+%i,%i,0); returned %i (%s)", socket,(void*)buf,bytes_done,bytes_left,errno,strerror(errno));
...
int send_timeout(int socket, void *buf, int len, unsigned int timeout) {
	...
	while (1) {
		...
		if (send_result < 0) {
			logmsg(lt_error,"send(%i,((char*)%x)+%i,%i,0); returned %i (%s)", socket,(void*)buf,bytes_done,bytes_left,errno,strerror(errno));
			// This is in most cases a critical error, so stop the loop
			return ERROR_SOCKET;
